...
class AccountPropertyTable extends PropertyTable
{
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('sold_status', function ($item) {
                if ($item->is_sold) {
                    if ($item->sold_expire_date && date('Y-m-d H:i:s', strtotime($item->sold_expire_date)) < date('Y-m-d H:i:s')) {
                        return Html::tag('span', trans('plugins/real-estate::property.sold_label'), ['class' => 'text-danger'])->toHtml();
                    }
                }
                return '';
            })
    ...
    public function query()
    {
        $query = $this->repository->getModel()
            ->select([
                ...
                're_properties.is_sold',
                're_properties.sold_expire_date',
    ...
    public function columns()
    {
        $columns  = [
            ...
            'sold_status' => [
                'name'  => 're_properties.is_sold',
                'title' => trans('plugins/real-estate::property.sold_status'),
                'width' => '100px',
            ],
        ];
