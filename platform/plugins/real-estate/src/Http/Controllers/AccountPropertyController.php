...
class AccountPropertyController extends Controller
{
    ...
    public function edit($id, FormBuilder $formBuilder, Request $request)
    {
        ...
        if ($property->is_sold) {
            ...
            if ($property->sold_expire_date && date('Y-m-d H:i:s', strtotime($property->sold_expire_date)) < date('Y-m-d H:i:s')) {
                return back()->with(['error_msg' => trans('plugins/real-estate::account-property.sold_property_alert')]);
            }
        }
    ...
    public function update(
        ...
        if ($property->sold_expire_date == null && $request->input('is_sold') == 1) {
            $request->merge(['sold_expire_date' => now()->addDays(RealEstateHelper::soldPropertyShowDays())]);
        }
    ...
    public function getProperty($id, BaseHttpResponse $response)
    {
        ...
        if ($property->is_sold) {
            if ($property->sold_expire_date && date('Y-m-d H:i:s', strtotime($property->sold_expire_date)) < date('Y-m-d H:i:s')) {
                return $response->setError(true)->setMessage(trans('plugins/real-estate::account-property.sold_property_alert'));
            }
        } 
        ...
        if ($expire_date >= $now || $property->never_expired) {
            return $response->setError(true)->setMessage(trans('plugins/real-estate::account-property.property_notexpired_alert'));
        }
