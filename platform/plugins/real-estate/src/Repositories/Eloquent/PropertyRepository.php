                ...
                ->where(function ($query) use ($filters) {
                    ...
                    if ($minPrice !== null) {
                        $query = $query->whereRaw("
                            IF (`re_properties`.`discount_rate` > 0, `re_properties`.`discount_rate` >= {$minPrice}, `re_properties`.`price` >= {$minPrice})
                        ");
                    }

                    if ($maxPrice !== null) {
                        $query = $query->whereRaw("
                            IF (`re_properties`.`discount_rate` > 0, `re_properties`.`discount_rate` <= {$maxPrice}, `re_properties`.`price` <= {$maxPrice})
                        ");
                    }
